...
app.post('/api/create-acl',
  authenticateToken,
  requirePermission('write_acls'),
  [
    body('aclNumber').isInt({ min: 1, max: 199 }).withMessage('ACL Number must be an integer between 1 and 199'),
  ...
  async (req, res) => {
    try {
      // For now, just echo back the request for debugging
      res.json({ message: 'Received ACL request', data: req.body });
    } catch (err) {
      console.error('ACL creation error:', err);
      res.status(500).json({ error: 'Failed to create ACL', details: err.message });
    }
  }
);
