...
app.get('/api/security-monitor/status', (req, res) => {
  try {
    const syslogPath = path.join(__dirname, 'syslog_data.json');
    ...
    const syslogData = JSON.parse(fs.readFileSync(syslogPath, 'utf-8'));
    const status = {
      status: 'active',
      totalLogs: syslogData.logs ? syslogData.logs.length : 0,
      threatsDetected: syslogData.threats ? syslogData.threats.length : 0,
      alertsGenerated: syslogData.alerts ? syslogData.alerts.length : 0,
      lastUpdate: syslogData.last_update || null,
      switchIp: '192.168.111.198',
      ollamaAvailable: true
    };
    res.json(status);
  } catch (error) {
    console.error('Security monitor status error:', error);
    res.status(500).json({ error: 'Failed to get security monitor status' });
  }
});

// Get security monitor logs
app.get('/api/security-monitor/logs', (req, res) => {
  try {
    const syslogPath = path.join(__dirname, 'syslog_data.json');
    if (!fs.existsSync(syslogPath)) {
    ...
    const syslogData = JSON.parse(fs.readFileSync(syslogPath, 'utf-8'));
    const logs = syslogData.logs || [];
    
    // Return logs in reverse chronological order (newest first)
    const sortedLogs = logs.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
    res.json({ logs: sortedLogs });
  } catch (error) {
    console.error('Security monitor logs error:', error);
    res.status(500).json({ error: 'Failed to get security monitor logs' });
  }
});

// Get security monitor threats
app.get('/api/security-monitor/threats', (req, res) => {
  try {
    const syslogPath = path.join(__dirname, 'syslog_data.json');
    if (!fs.existsSync(syslogPath)) {
    ...
    const syslogData = JSON.parse(fs.readFileSync(syslogPath, 'utf-8'));
    const threats = syslogData.threats || [];
    
    // Return threats in reverse chronological order (newest first)
    const sortedThreats = threats.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
    res.json({ threats: sortedThreats });
  } catch (error) {
    console.error('Security monitor threats error:', error);
    res.status(500).json({ error: 'Failed to get security monitor threats' });
  }
});

// Get security monitor alerts
app.get('/api/security-monitor/alerts', (req, res) => {
  try {
    const syslogPath = path.join(__dirname, 'syslog_data.json');
    if (!fs.existsSync(syslogPath)) {
    ...
    const syslogData = JSON.parse(fs.readFileSync(syslogPath, 'utf-8'));
    const alerts = syslogData.alerts || [];
    ...
    console.error('Security monitor alerts error:', error);
    res.status(500).json({ error: 'Failed to get security monitor alerts' });
  }
});

// Start security monitoring
app.post('/api/security-monitor/start', (req, res) => {
  try {
    // Check if syslog data exists
    const syslogPath = path.join(__dirname, 'syslog_data.json');
    if (!fs.existsSync(syslogPath)) {
      return res.json({
        status: 'started',
    ...
    res.json({
      ...
      threats: syslogData.threats || [],
      alerts: syslogData.alerts || []
    });
  } catch (error) {
    console.error('Security monitor start error:', error);
    res.status(500).json({ error: 'Failed to start security monitoring' });
  }
});

// Stop security monitoring
app.post('/api/security-monitor/stop', (req, res) => {
  try {
    res.json({
      status: 'stopped',
    ...
    console.error('Security monitor stop error:', error);
    res.status(500).json({ error: 'Failed to stop security monitoring' });
  }
});
